/**
 * @file JS8.cpp
 * @brief JS8 modem core: encoder and multi-submode synchronous decoder.
 *
 * Implements the full JS8 modem pipeline:
 *   - 8-FSK encoder (genjs8): packs a 12-character message into 79 tones
 *     using LDPC forward-error-correction, CRC-12, and Costas pilot arrays.
 *   - Sync search (syncjs8): scans the spectrum for Costas-array pilot
 *     correlations and builds a ranked candidate list.
 *   - Decoder (js8dec): fine-synchronises each candidate, computes LLRs via
 *     8-FSK demapping, runs the BP LDPC decoder, and emits decoded frames.
 *
 * Rewritten from the JS8Call-improved GFSK8_Mode/JS8.cpp with Qt and FFTW3
 * replaced by KissFFT (via fft_shim.h) and std:: equivalents.
 *
 * Original C++ conversion: (C) 2025 Allan Bazinet <w6baz@arrl.net>
 * Modifications for standalone library: gfsk8modem project
 */
...
#include <cstdlib>
#include "fft_shim.h"
#include "crc12.h" // from vendor/crc12.h
...
constexpr auto ZERO_CMPLX = std::complex<float>{0.0f, 0.0f};
...
struct ModeA {
    ...
    inline static constexpr auto NCOSTAS = GFSK8::Costas::Type::ORIGINAL;
    inline static constexpr int NSPS = GFSK8_NORMAL_SYMBOL_SAMPLES;
    inline static constexpr int NTXDUR = GFSK8_NORMAL_TX_SECONDS;
    ...
    inline static constexpr int NMAX = NTXDUR * GFSK8_RX_SAMPLE_RATE;
...
struct ModeB {
    ...
    inline static constexpr auto NCOSTAS = GFSK8::Costas::Type::MODIFIED;
    inline static constexpr int NSPS = GFSK8_FAST_SYMBOL_SAMPLES;
    inline static constexpr int NTXDUR = GFSK8_FAST_TX_SECONDS;
...
struct ModeC {
    ...
    inline static constexpr auto NCOSTAS = GFSK8::Costas::Type::MODIFIED;
    inline static constexpr int NSPS = GFSK8_TURBO_SYMBOL_SAMPLES;
    inline static constexpr int NTXDUR = GFSK8_TURBO_TX_SECONDS;
...
struct ModeE {
    ...
    inline static constexpr auto NCOSTAS = GFSK8::Costas::Type::MODIFIED;
    inline static constexpr int NSPS = GFSK8_SLOW_SYMBOL_SAMPLES;
    inline static constexpr int NTXDUR =
        GFSK8_SLOW_TX_SECONDS; // XXX was 28 in Fortran
    inline static constexpr int NDOWNSPS = 32;
...
struct ModeI {
    ...
    inline static constexpr auto NCOSTAS = GFSK8::Costas::Type::MODIFIED;
    inline static constexpr int NSPS = GFSK8_ULTRA_SYMBOL_SAMPLES;
    inline static constexpr int NTXDUR = GFSK8_ULTRA_TX_SECONDS;
    inline static constexpr int NDOWNSPS = 12;
...
template <std::floating_point T> class KahanCompensatedSum {
    T m_sum;          // Accumulated sum
    T m_compensation; // Compensation for lost low-order bits

  public:
    KahanCompensatedSum(T sum = T(0)) : m_sum(sum), m_compensation(T(0)) {}

    KahanCompensatedSum &operator=(T const sum) {
        m_sum = sum;
        m_compensation = T(0);
    ...
    KahanCompensatedSum &operator+=(T const value) {
...
class FftPlanSet {
  public:
    ...
    FftPlanSet(FftPlanSet const &) = delete;
    FftPlanSet &operator=(FftPlanSet const &) = delete;
    FftPlanSet(FftPlanSet &&) = delete;
    FftPlanSet &operator=(FftPlanSet &&) = delete;

    // Constructor

    FftPlanSet() { m_plans.fill(nullptr); }
...
struct SyncPeak {
    ...
    SyncPeak(float const freq, float const step, float const sync)
        : freq(freq), step(step), peakValue(sync) {}
};
...
// Sync candidate list used by syncjs8(); sorted/searched as needed.
// Replaces the boost::multi_index_container that provided three simultaneous
// views (by freq, by rank, by sync descending). With NMAXCAND=300, the
// small size makes std::vector + sort-on-demand equally efficient.

using SyncIndex = std::vector<SyncPeak>;
...
template <typename T> std::uint16_t CRC12(T const &range) {
    return crc12::compute(reinterpret_cast<const uint8_t*>(range.data()), range.size()) ^ 42;
}
...
template <typename Mode> class DecodeMode {
    ...
    FftPlanSet plans;
    SyncIndex m_candidates;
    gfsk8::SoftCombiner<N> m_softCombiner;
    bool m_enableFreqTracking = true;
    bool m_enableTimingTracking = true;
    float m_llrErasureThreshold = gfsk8::llrErasureThreshold();
    bool m_enableLdpcFeedback = gfsk8::ldpcFeedbackEnabled();
    int m_maxLdpcPasses = gfsk8::ldpcFeedbackMaxPasses();

    using Plan = FftPlanSet::Type;

    static constexpr auto Costas = GFSK8::Costas::array(Mode::NCOSTAS);
    ...
    // N = BASELINE_DEGREE + 1 = 6 nodes, 6 polynomial coefficients.

    static constexpr int BASELINE_N = static_cast<int>(BASELINE_NODES.size());

    // p[i] = {x_i, y_i} — sample points
    // V[i][j] = x_i^j   — Vandermonde matrix (row-major)
    // c[j]               — polynomial coefficients

    std::array<std::array<double, 2>, BASELINE_N> p;
    std::array<std::array<double, BASELINE_N>, BASELINE_N> V;
    std::array<double, BASELINE_N> c;

    // Polynomial evaluation using Estrin's method (pairs of coefficients).
    // Equivalent to c[0] + c[1]*x + c[2]*x^2 + ... + c[N-1]*x^(N-1).

    inline float evaluate(float const x) const {
        double baseline = 0.0, exponent = 1.0;
        for (int i = 0; i < BASELINE_N / 2; ++i) {
            baseline += (c[i * 2] + c[i * 2 + 1] * static_cast<double>(x)) * exponent;
            exponent *= static_cast<double>(x) * static_cast<double>(x);
        }
    ...
    std::optional<Decode> js8dec(bool const syncStats, bool const lsubtract,
                                 ...
                                 float &xsnr, GFSK8::Event::Emitter emitEvent) {
        ...
        if (syncStats)
            emitEvent(
                GFSK8::Event::SyncState{GFSK8::Event::SyncState::Kind::CANDIDATE,
                                      Mode::NSUBMODE,
                                      f1,
                                      xdt,
                                      {.candidate = nsync}});
        ...
        auto const whitening = gfsk8::WhiteningProcessor<NROWS, ND, N>::process(
        ...
        auto const tryDecode = [&](std::array<float, N> const &llrInput,
            ...
            if (nharderrors >= 0 && nharderrors < 60 &&
                ...
                if (checkCRC12(decoded)) {
                    if (syncStats)
                        emitEvent(GFSK8::Event::SyncState{
                            GFSK8::Event::SyncState::Kind::DECODED,
                            Mode::NSUBMODE,
                            f1,
                            xdt2,
                            {.decoded = sync}});
                    ...
                    GFSK8::encode(i3bit, Costas, message.data(), itone.data());
        ...
        for (int ipass = 1; ipass <= 4 && totalLdpcPasses < m_maxLdpcPasses;
            ...
            if (m_enableLdpcFeedback && totalLdpcPasses < m_maxLdpcPasses) {
                ...
                int confident = 0;
                int uncertain = 0;
                gfsk8::refineLlrsWithLdpcFeedback(
    ...
    void baselinejs8(int const ia, int const ib) {
        ...
        // Solve V*c = y by Gaussian elimination with partial pivoting.
        ...
        for (int i = 0; i < BASELINE_N; ++i) {
            double xi = p[i][0];
            A[i][0] = 1.0;
            for (int j = 1; j < BASELINE_N; ++j)
                A[i][j] = A[i][j - 1] * xi;
            A[i][BASELINE_N] = p[i][1];
        }

        // Forward elimination with partial pivoting
        for (int col = 0; col < BASELINE_N; ++col) {
            int pivot = col;
            for (int row = col + 1; row < BASELINE_N; ++row)
            ...
            for (int row = col + 1; row < BASELINE_N; ++row) {
                ...
                for (int j = col; j <= BASELINE_N; ++j)
                    A[row][j] -= factor * A[col][j];
            }
        }

        // Back substitution
        for (int row = BASELINE_N - 1; row >= 0; --row) {
            if (std::abs(A[row][row]) < 1e-14) { c[row] = 0.0; continue; }
            c[row] = A[row][BASELINE_N];
            for (int j = row + 1; j < BASELINE_N; ++j)
                c[row] -= A[row][j] * c[j];
            c[row] /= A[row][row];
        }
    ...
    void js8_downsample(float const f0) {
        ...
        std::fill_n(cd0.begin(), Mode::NDFFT2, ZERO_CMPLX);
    ...
    std::vector<SyncPeak> syncjs8(int nfa, int nfb) {
        ...
        for (int i = ia; i <= ib; ++i) {
            ...
            m_candidates.emplace_back(Mode::DF * i, Mode::TSTEP * (max_index + 0.5f),
                              max_value);
        }
        ...
        // Normalize all sync values to the 40th percentile.
        // Equivalent to the ranked_index.nth(size*4/10)->sync approach.

        {
            std::vector<float> syncVals;
            syncVals.reserve(m_candidates.size());
            for (auto const &s : m_candidates) syncVals.push_back(s.peakValue);

            auto nth = syncVals.begin() +
                       static_cast<std::ptrdiff_t>(syncVals.size() * 4 / 10);
            std::nth_element(syncVals.begin(), nth, syncVals.end());
            float const p40 = *nth;

            if (p40 != 0.0f) {
                for (auto &s : m_candidates) s.peakValue /= p40;
            }
        }

        // Sort by sync descending to iterate highest-first.

        std::sort(m_candidates.begin(), m_candidates.end(),
                  [](SyncPeak const &a, SyncPeak const &b) { return a.peakValue > b.peakValue; });

        // Greedy candidate selection: take the strongest remaining candidate,
        // then suppress all entries within ±AZ Hz of its frequency.
        // This replicates the freqIndex.erase() behaviour of the original.

        std::vector<bool> suppressed(m_candidates.size(), false);
        std::vector<SyncPeak> candidates;

        for (std::size_t i = 0; i < m_candidates.size() && candidates.size() < NMAXCAND; ++i) {
            if (suppressed[i]) continue;

            // Stop if below threshold or invalid.
            if (m_candidates[i].peakValue < ASYNCMIN || std::isnan(m_candidates[i].peakValue))
                break;

            candidates.push_back(m_candidates[i]);

            // Suppress nearby frequencies.
            float const chosen_freq = m_candidates[i].freq;
            for (std::size_t k = i + 1; k < m_candidates.size(); ++k) {
                if (std::abs(m_candidates[k].freq - chosen_freq) <= Mode::AZ)
                    suppressed[k] = true;
            }
        }
    ...
    void subtractjs8(std::vector<std::complex<float>> const &cref,
        ...
        std::fill(cfilt.begin() + size, cfilt.end(), ZERO_CMPLX);
  ...
  public:
    ...
    DecodeMode() {
        m_enableFreqTracking =
            std::getenv("GFSK8_DISABLE_FREQ_TRACKING") == nullptr;
        m_enableTimingTracking =
            std::getenv("GFSK8_DISABLE_TIMING_TRACKING") == nullptr;
    ...
    std::size_t operator()(RxAudioBuffer const &data, int const kpos,
                           int const ksz, GFSK8::Event::Emitter emitEvent) {
        ...
        if (data.params.syncStats)
            emitEvent(GFSK8::Event::SyncStart{pos, sz});
        ...
        if ((GFSK8_RX_SAMPLE_SIZE - pos) < sz) {
            ...
            int const firstsize = GFSK8_RX_SAMPLE_SIZE - pos;
        ...
        for (int ipass = 1; ipass <= 3; ++ipass) {
            ...
            for (auto [f1, xdt, peakSync] : candidates) {
                float xsnr = 0.0f;
                int nharderrors = -1;
...
/******************************************************************************/
// Decoder — synchronous (replaces Qt Worker/QThread)
/******************************************************************************/
...
class Decoder::Impl {
  ...
  public:
    explicit Impl(RxAudioBuffer &data) : m_data(data) {}

    void operator()(Event::Emitter const &emitEvent) {
...
Decoder::Decoder() : m_impl(std::make_unique<Impl>(m_data)) {}

Decoder::~Decoder() = default;

void Decoder::snapshot() { m_data = dec_data; }

void Decoder::decode(Event::Emitter const &emitEvent) {
    (*m_impl)(emitEvent);
}
